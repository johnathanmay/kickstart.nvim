...
-- Ensure Go binaries are available to Mason (which spawns subprocesses without full shell PATH)
...
require('lazy').setup({
  ...
  -- JM: catppuccin theme with auto dark/light mode switching
  {
  ...
  --   },
  -- }
  -- JM: copilot options (left for reference)
  --{
      ...
      local servers = {
        ...
        terraformls = {
          ...
          on_init = function(client)
            -- TODO: remove once hashicorp/terraform-ls#2108 ships a fix (tracked
            -- by PR #2095). Multi-line semantic tokens (e.g. heredoc strings) can
            -- encode a negative deltaStart that wraps to a huge uint32, which hangs
            -- the nvim client at 100% CPU decoding it. Disable until fixed.
  ...
  -- JM: add highlight color for JM keyword
  {
  ...
  { -- Highlight, edit, and navigate code
    ...
    config = function()
      ...
      local parsers = { 'bash', 'c', 'diff', 'go', 'hcl', 'html', 'lua', 'luadoc', 'markdown', 'markdown_inline', 'query', 'terraform', 'vim', 'vimdoc' }
